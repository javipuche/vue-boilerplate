...
export default class Header {
    constructor (container) {
        ...
        this.megamenus = this.el.querySelectorAll(this.selectors.megamenu)
    ...
    bindNavigationLinkEvents () {
        ...
        if (navigationLink.length) {
            navigationLink.forEach((link) => link.addEventListener('click', this.toggleClickNavigation.bind(this)))
            navigationLink.forEach((link) => link.addEventListener('mouseover', this.toggleHoverNavigation.bind(this)))
        }
    }
    ...
    isMegaMenuActive () {
        return [...this.megamenus].some(megamenu => megamenu.closest(this.selectors.navigationLink).classList.contains(activeClass)) || this.mobileNavigation.classList.contains(activeClass)
    }
    ...
    toggleClickNavigation (event) {
        const isMegamenuEnabled = window.getComputedStyle(this.megamenu).getPropertyValue('--js-megamenu-enable')
        const target = event.target

        if (target.tagName === 'NAV' || target.tagName === 'UL' || isMegamenuEnabled) return
    ...
    toggleHoverNavigation ({ target }) {
        ...
        if (isMegamenuEnabled) {
            this.genericDelay(() => {
                ...
                if (!element.classList.contains(activeClass)) this.selectDefaultNavigation(element)

                element.classList.add(activeClass)

                if (this.hasDefaultOpenNavigation()) overlay.showOverlay()
            }, 300)
        }
    }
    ...
    genericDelay (fn, ms) {
        ...
        this.timer = setTimeout(fn, ms || 500)
    };
}
